...
func TestWorkloadAwarePreemptionScoring(t *testing.T) {
	...
	pl, err := New(ctx, getDefaultDefaultPreemptionArgs(), f, feature.Features{EnableGenericWorkload: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	preemptState := preemption.NewPodGroupPreemptionState(allVictims, nil)
