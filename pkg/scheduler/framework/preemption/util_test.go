...
func TestPodGroupPreemptionState(t *testing.T) {
	...
	state := NewPodGroupPreemptionState([]*DomainVictim{multiNodePG, singleNodePod}, nil)
	...
	state.AddCondemned([]*DomainVictim{multiNodePG})
	...
	state.AddCondemned([]*DomainVictim{multiNodePG})
	state.RemoveCondemned([]*DomainVictim{multiNodePG})
	if got := len(state.SurvivingVictimsOnNode("node2")); got != 0 {
